...
class Admin extends CI_Controller
{
	...
	{
		if ($this->session->userdata('status') == 'login' && $this->session->userdata('role') == 1) {
			$data['stokBarangMasuk'] = $this->M_admin->numrows('tb_barang_masuk');
			$data['stokBarangKeluar'] = $this->M_admin->numrows('tb_barang_keluar');
	...
	public function proses_tambahuser()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			redirect(base_url('admin/tambah_users'));
		} else {
	...
	public function proses_gambarupload()
	{
		...
		if (!$this->upload->do_upload('userpicture')) {
			...
			$this->M_admin->update_gambar($where, $data);
	...
	{
		$data['list_data'] = $this->M_admin->select('tb_serv_genset');
		$data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
	...
	public function proses_tambah_service_genset()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			$spare_part = $this->input->post('spare_part', true);
	...
	{
		$uri = $this->uri->segment(3);
		$where = array('kode_genset' => $uri);

		$count_kode_genset = $this->M_admin->numrows_where('tb_serv_genset', $where);
		if ($count_kode_genset == 0) {

			$data['list_data'] = $this->M_admin->get_data_tb('tb_genset', $where);
			$data['avatar'] = $this->M_admin->get_data_gambar('tb_upload_gambar_user', $this->session->userdata('name'));
			$data['title'] = 'Update Perbaikan Genset';
			$this->load->view('admin/form_service_genset/update_service_genset_detail', $data);
		} else {
